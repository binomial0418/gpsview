...
- 📍 **軌跡回放**：支援播放歷史軌跡，可調整播放速度（0.5x - 20x）
- 🔐 **安全認證**：bcrypt 密碼雜湊、暴力破解防護、Session 防劫持
...
├── debug.php                  # 除錯工具
├── get-gps-to-db.php         # GPS 資料匯入工具，請接上你的軌跡點來源，再呼叫本程式將軌跡資料新增至DB
...
首次存取時會看到登入畫面，輸入帳號與密碼：

- **Session 逾時**：1 小時後自動登出

### 查詢軌跡

1. **按時間**：選擇「最近 2/4/6/8/12/16/24 小時」
2. **按日期**：輸入特定日期查詢該日軌跡
3. **自動更新**：勾選「自動更新」並選擇刷新間隔（5-60 秒）
...
define('DB_NAME', 'gpsdb');
define('DB_TABLE', 'gpslog');
```

## 資料表結構

GPS 資料表 `gpslog` 需包含以下欄位：
...
3. **設定權限**：
   ...
   chmod 755 generate_password_hash.php
   ```
4. **啟用 HTTPS**：生產環境應使用 HTTPS 並在 `gps_view.php` 啟用 Secure cookie flag

### 備份

定期備份 `.env.php`（包含重要的帳號設定）：

```bash
# 本地備份
cp .env.php .env.php.backup.$(date +%Y%m%d_%H%M%S)
```
